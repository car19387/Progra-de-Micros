...
reiniciar_tmr0 macro ; Reinicio de Timer0
    ...
    bcf ((INTCON) and 07Fh), 2 ; Limpiar bit de interrupción por overflow (Bit de INTCON)
    endm
...
isr:
    btfsc ((INTCON) and 07Fh), 2 ; Testear si esta encendida la bandera para...
    call int_tmr0 ; Ir a la subrutina del TMR0
    btfsc ((INTCON) and 07Fh), 0 ; Testear si esta encendida la bandera para...
...
int_IOCB:
    ...
    bcf ((INTCON) and 07Fh), 0 ; Limpiar la bandera de IOC
    return
...
config_reloj: ; Configuración de reloj interno
    ...
    bcf ((OSCCON) and 07Fh), 4 ; Configuración del oscilador a 4MHz
...
config_tmr0: ; Configuración de Timer0
    ...
    bcf ((OPTION_REG) and 07Fh), 5 ; Seleccion entre reloj int. o ext. (Bit de OPTION_REG)
    bcf ((OPTION_REG) and 07Fh), 3 ; Prescaler asignado a Timer0 (Bit de OPTION_REG)
    bsf ((OPTION_REG) and 07Fh), 2
    bsf ((OPTION_REG) and 07Fh), 1
    bsf ((OPTION_REG) and 07Fh), 0 ; Bits para prescaler (1:256) (Bits de OPTION_REG)
