    ...
    while(1){
        if (PIR1bits.TXIF) {
            ...
            if (menu == 49) {
                indicador = 1;
                printf("Val art base: ");
                printval(val_articulacion_base);
                printf("    Val art codo: ");
                printval(val_articulacion_codo);
                printf("    Val pinza: ");
                printval(val_pinza);
                printf("    Val giro: ");
                printval(val_giro);
...
void __attribute__((picinterrupt(("")))) isr(void) {
    if (PIR1bits.ADIF) {
        if(ADCON0bits.CHS == 0) {
            val_articulacion_base = ADRESH;
        }

        if(ADCON0bits.CHS == 1) {
            val_articulacion_codo = ADRESH;
        }
    ...
    if (INTCONbits.T0IF) {
        ...
        if (ban_delay == 3) {
            TMR0 = 255-((val_pinza>>1)+120);
            PORTBbits.RB0 = !PORTBbits.RB0;
            PORTBbits.RB1 = 0;
            INTCONbits.T0IF = 0;
            ban_delay = ban_delay+1;
        }

        if (ban_delay == 2) {
            TMR0 = 255-((val_articulacion_codo>>1)+120);
        ...
        if (ban_delay == 1) {
            TMR0 = 255-((val_articulacion_base>>1)+120);
...
void printval(char valor){
    char centenas = valor/100;
    char decenas_temp = valor%100;
    char decenas = decenas_temp/10;
    char unidades = valor%10;
    TXREG = tab7seg[centenas];
    _delay((unsigned long)((500)*(1000000/4000000.0)));
    TXREG = tab7seg[decenas];
    _delay((unsigned long)((500)*(1000000/4000000.0)));
    TXREG = tab7seg[unidades];
    _delay((unsigned long)((500)*(1000000/4000000.0)));
}
